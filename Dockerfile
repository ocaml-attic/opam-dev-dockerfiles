...
LABEL distro_style="zypper" distro="opensuse" distro_long="opensuse-42.2" arch="x86_64" ocaml_version="4.06.0+flambda" opam_version="master" operatingsystem="linux"
RUN git clone -b master git://github.com/ocaml/opam /tmp/opam && \
  sh -c "cd /tmp/opam && make cold && make prefix=\"/usr\" cold-install && echo Not installing OPAM2 wrappers && rm -rf /tmp/opam" && \
