...
LABEL distro_style="yum" distro="centos" distro_long="centos-7" arch="x86_64" ocaml_version="4.05.0+flambda" opam_version="master" operatingsystem="linux"
RUN rpm --rebuilddb && yum install -y sudo passwd bzip2 gcc-c++ patch nano git which tar wget xz openssl centos-release-xen && yum clean all && \
...
RUN mkdir .ssh && \
  ...
  sudo -u opam sh -c "opam init -a -y --comp ocaml-variants.4.05.0+flambda /home/opam/opam-repository" && \
