...
LABEL distro_style="yum" distro="fedora" distro_long="fedora-25" arch="x86_64" ocaml_version="4.05.0+flambda" opam_version="master" operatingsystem="linux"
RUN rpm --rebuilddb && yum install -y sudo passwd bzip2 gcc-c++ patch nano git which tar wget xz openssl redhat-rpm-config && yum clean all && \
