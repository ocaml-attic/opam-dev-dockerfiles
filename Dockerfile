...
LABEL distro_style="yum" distro="fedora" distro_long="fedora-25" arch="x86_64" ocaml_version="4.00.1" opam_version="master" operatingsystem="linux"
RUN rpm --rebuilddb && yum install -y sudo passwd bzip2 patch nano git which tar wget xz openssl redhat-rpm-config && yum clean all && \
...
RUN mkdir .ssh && \
  ...
  sudo -u opam sh -c "opam init -a -y --comp ocaml-base-compiler.4.00.1 /home/opam/opam-repository" && \
